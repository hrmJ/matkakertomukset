...
deprels$root$where <- deprels$root$df$sentence[grepl("asu[a-ö]+ [a-ö]+ssa",deprels$root$df$sentence, ignore.case=T)]

deprels$root$df$sentence[!(deprels$root$df$sentence %in% deprels$root$where)]
...
[Topic indicators
    ...
    [root (\Sexpr{deprels$props["root"]} \%)
        [First person (\Sexpr{deprels$root$pers$prop["1"]} \%)
            [asua + locative adjunct (\Sexpr{round(length(deprels$root$where) / nrow(deprels$root$df[deprels$root$df$headverb_person=="1",])*100)}\%)
